...
├── __init__.py                      # Makes the root folder a Python package
│
├── data/
│   ├── raw/                         # Source CSV files for ingestion
│   │   ├── customers.csv
│   │   ├── drugs.csv
│   │   ├── drug_batches.csv
...
│   │   └── suppliers.csv
│   │
│   └── reports/                      # CSV outputs generated by the ETL pipeline
...
│   ├── pharma_ingestion_validation.sql # Referential integrity checks for ingested data
│   └── sanity_checks.sql             # Business-rule compliance checks
│
├── docs/
│   └── data_dictionary.md            # Documentation of tables, fields, and types
│
├── flows/                            # ETL orchestration logic using Prefect
│   ├── master_flow.py                # Orchestrates full ETL pipeline
...
│   ├── load_data.py                  # Loads raw CSV files into MySQL
│   ├── pandas_analytics.py           # Pandas-based fact and aggregation computations
│   ├── persist_sql_outputs.py        # Persists SQL query results to CSV
│   ├── sanity_check_ingestion.py     # Validates ingestion and business rules
│   ├── sql_analytics.py              # SQL-based fact and aggregation computations
│   ├── sql_executor.py               # Executes SQL scripts safely in MySQL
│   ├── validation.py                 # Compares SQL vs Pandas aggregation results
│   └── __init__.py                   # Makes tasks folder a Python package
```
