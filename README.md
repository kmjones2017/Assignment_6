...
Assignment_6/
├── .env.example
│ # Example environment file for database connection credentials
├── .env
│ # Local environment file (ignored by GitHub)
├── .gitignore
...
├── init.py
│ # Makes the root folder a Python package
│
├── data/
│ ├── raw/
│ │ # Raw CSV files used for ingestion
│ │ ├── customers.csv
│ │ ├── drugs.csv
│ │ ├── drug_batches.csv
│ │ ├── drug_formulations.csv
│ │ ├── orders.csv
│ │ ├── order_items.csv
│ │ ├── raw_materials.csv
│ │ └── suppliers.csv
│ │
│ └── reports/
│ # Final CSV outputs generated by the ETL pipeline
│ ├── orders_aggregates_result_1.csv
│ ├── orders_aggregates_result_2.csv
│ ├── orders_aggregates_result_3.csv
│ └── orders_aggregates_result_4.csv
│
├── db/
│ # SQL scripts for schema, ingestion, and aggregation
│ ├── load_data.sql
│ │ # Loads raw CSV data into MySQL
│ ├── orders_fact.sql
│ │ # Creates SQL fact table
│ ├── orders_aggregates.sql
│ │ # SQL aggregation queries to be persisted
│ ├── pharma_schema.sql
│ │ # Creates the database and tables
│ ├── pharma_ingestion_validation.sql
│ │ # Referential integrity checks for ingested data
│ └── sanity_checks.sql
│ # Validates business rules after ingestion
│
├── docs/
│ └── data_dictionary.md
│ # Documentation of tables, fields, and data types
│
├── flows/
│ # Prefect flows for ETL orchestration
│ ├── master_flow.py
│ │ # Orchestrates full ETL pipeline
│ ├── sql_analytics_subflow.py
│ │ # SQL-based fact table and aggregation logic
│ └── pandas_analytics_subflow.py
│ # Pandas-based fact table and aggregation logic
│
├── tasks/
...
│ ├── load_data.py
│ │ # Loads raw CSV files into MySQL
│ ├── pandas_analytics.py
│ │ # Performs fact table and aggregations using Pandas
│ ├── persist_sql_outputs.py
│ │ # Persists SQL query results to CSV
│ ├── sanity_check_ingestion.py
│ │ # Validates ingestion process and business rules
│ ├── sql_analytics.py
│ │ # Performs fact table and aggregations using SQL
│ ├── sql_executor.py
│ │ # Executes SQL scripts safely in MySQL
│ ├── validation.py
│ │ # Compares SQL vs Pandas aggregation results
│ └── init.py
│ # Makes the tasks folder a Python package
```
